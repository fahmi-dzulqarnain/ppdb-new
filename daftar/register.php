...
if (isset($_POST['send']) && isset($_POST['tgl-lahir'])) {
  ...
  $nSiswa = strtoupper($_POST['nama-siswa']);
  ...
  $nAyah = strtoupper($_POST['nama-ayah']);
  $nIbu = strtoupper($_POST['nama-ibu']);
  ...
  if ($checkIfExists->num_rows) {
    ...
    if ($isPermitted = true) {
      if (strpos($unit, 'TKIT') !== false) {
        ...
        $asalSekolah = strtoupper($_POST['asal-sekolah']);
        $rataRapor1 = $_POST['rataRapor1'];
        $rataRapor2 = $_POST['rataRapor2'];
        $rataRapor3 = $_POST['rataRapor3'];
        $rataRapor4 = $_POST['rataRapor4'];
        ...
        if (
          ...
          if ($tglLhr != '') {
            ...
            $getSchool = $mysqli->query("SELECT * FROM tbl_school WHERE school_name = '$unit'") or die($mysqli->error);
            $schoolData = $getSchool->fetch_array();
            $no_pendf = $schoolData['no_pend_terakhir'] + 1;
            $kuota = $schoolData['sisa_kuota'] - 1;
            $nominalTrf = number_format(strval(219000 + $no_pendf), 0, ",", ".");
            ...
            $mysqli->query("UPDATE tbl_school SET no_pend_terakhir = '$no_pendf', sisa_kuota = '$kuota' WHERE school_name = '$unit'") or die($mysqli->error);
